...
<!--      </div>-->
    ...
    <ul v-if="computeList.length" class="course-list">
      <li v-for="(item,index) in computeList" :key="index" class="list-item  col-md-9 col-sm-6 col-xs-12" @click="handleCourseClick(item.id)" style="margin-left: 10px">
        <div class="img-box">
          <img :src="item.img" alt="">
          <div class="tags">
            <span v-for="(tag, index) in item.tags" :key="index" class="tag-item">{{ tag.name }}</span>
          </div>
        </div>
        <div class="course-content">
          ...
          <p>
            <span class="price">{{ item.price===0?'FREE':item.price }}</span>
